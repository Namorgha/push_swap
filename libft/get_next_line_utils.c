...
/*   Updated: 2023/02/08 00:37:00 by namorgha         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*strjoin(char *s1, char *s2)
{
	...
	i = 0;
	j = 0;
	if (ft_strleen(s1) == 0 && ft_strleen(s2) == 0)
		return (0);
	ptr = malloc(sizeof(char) * (ft_strleen(s1) + ft_strleen(s2) + 1));
	if (!ptr)
		return (0);
	while (s1 && s1[i])
	{
	...
	while (s2 && i < ft_strleen(s1) + ft_strleen(s2))
		ptr[i++] = s2[j++];
	ptr[i] = '\0';
	free(s1);
	return (ptr);
